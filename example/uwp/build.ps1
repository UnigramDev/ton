...
function prepare {
  ...
  cmake $td_root -A win32 -DCMAKE_TOOLCHAIN_FILE="$vcpkg_cmake" -DTON_ONLY_TONLIB=ON -DTONLIB_ENABLE_DOTNET=ON
  CheckLastExitCode
...
function config {
  ...
  ForEach($arch in $arch_list) {
    ...
    cmake "$td_root" -A $fixed_arch -DCMAKE_SYSTEM_VERSION="10.0" -DCMAKE_SYSTEM_NAME="WindowsStore" -DCMAKE_TOOLCHAIN_FILE="$vcpkg_cmake" -DTON_ONLY_TONLIB=ON -DTONLIB_ENABLE_DOTNET=ON
    CheckLastExitCode
    cd ..
  }
  echo "done"
...
function export {
  ...
  ForEach($arch in $arch_list) {
    ...
    cp ${arch}/tonlib/Debug/* -include "LIBEAY*","SSLEAY*","zlib*" vsix/Redist/Debug/${arch}/
    cp ${arch}/tonlib/Release/* -include "LIBEAY*","SSLEAY*","zlib*" vsix/Redist/Retail/${arch}/

    cp ${arch}/tonlib/Debug/* -filter "Ton.Tonlib.*" -include "*.lib" vsix/DesignTime/Debug/${arch}/
    cp ${arch}/tonlib/Release/*  -filter "Ton.Tonlib.*" -include "*.lib" vsix/DesignTime/Retail/${arch}/

    cp ${arch}/tonlib/Debug/*  -filter "Ton.Tonlib.*" -include "*.pdb","*.dll" vsix/Redist/Debug/${arch}/
    cp ${arch}/tonlib/Release/*  -filter "Ton.Tonlib.*" -include "*.pdb","*.dll" vsix/Redist/Retail/${arch}/

    cp ${arch}/tonlib/Release/* -filter "Ton.Tonlib.*" -include "*.pri","*.winmd","*.xml" vsix/References/CommonConfiguration/${arch}/
  }
  ...
  if ($compress -eq "zip") {
    ...
    WinRAR.exe a -afzip -r -ep1 tonlib.vsix *
  } else {
    7z.exe a -tzip -r tonlib.vsix *
  }
  cd ..
}
