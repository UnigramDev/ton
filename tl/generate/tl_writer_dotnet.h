...
class TlWriterDotNet : public TL_writer {
 public:
  ...
  void gen_to_unmanaged(std::stringstream &ss, const tl_combinator *t) const {
    ...
    ss << "ton::tonlib_api::object_ptr<ton::tonlib_api::" << native_class_name << "> ToUnmanaged(" << class_name << "^ from)";
    if (is_header_) {
      ss << ";\n";
      return;
    }
    ss << "{\n"
       ...
       << "    return nullptr;\n"
       << "  }\n"
       << "  return ton::tonlib_api::make_object<ton::tonlib_api::" << native_class_name << ">(";
    bool is_first = true;
    for (auto &it : t->args) {
      if (is_first) {
        is_first = false;
      ...
      const tl_tree_type *arg_type = static_cast<const tl_tree_type *>(it.type);
      bool need2_secure = need_secure(arg_type);
      ss << (need2_secure ? "Secure" : "") << "ToUnmanaged(from->" << field_name << ")";
    }
    ss << ");\n}\n";
  }
  bool need_secure(const tl_tree_type *tree_type) const {
    const tl_type *t = tree_type->type;
    const std::string &name = t->name;
  ...
  void gen_from_unmanaged(std::stringstream &ss, const tl_combinator *t) const {
    auto native_class_name = gen_native_class_name(t->name);
    auto class_name = gen_class_name(t->name);
    ss << class_name << "^ FromUnmanaged(ton::tonlib_api::" << native_class_name << " &from)";
    if (is_header_) {
      ss << ";\n";
      return;
    }
  ...
  std::string gen_additional_proxy_function_begin(const std::string &function_name, const tl_type *type,
    ...
    if (function_name == "ToUnmanaged") {
      ...
      ss << "{\n"
         ...
         << "    return nullptr;\n"
         << "  }\n"
         << "  return ton::tonlib_api::move_object_as<ton::tonlib_api::" << native_class_name
