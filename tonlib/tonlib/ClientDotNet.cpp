...
public:
  ...
  void Send(Api::Function^ function, ClientResultHandler^ handler) {
    ...
    tonlib::Client::Request request;
    request.id = queryId;
    request.function = ton::tonlib_api::move_object_as<ton::tonlib_api::Function>(ToUnmanaged(function)->get_object_ptr());
  ...
  static Api::BaseObject^ Execute(Api::Function^ function) {
    ...
    request.id = 0;
    request.function = ton::tonlib_api::move_object_as<ton::tonlib_api::Function>(ToUnmanaged(function)->get_object_ptr());
    return Api::FromUnmanaged(*tonlib::Client::execute(std::move(request)).object);
  }
